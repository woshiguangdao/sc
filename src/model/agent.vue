    ...
    <div class="w-80 bg-white border-r border-gray-200 flex flex-col shadow-sm z-20">
      ...
      <div class="p-5 border-b border-gray-100">
        ...
        <div class="relative group">
          ...
          <div class="border-2 border-dashed border-blue-200 rounded-lg p-4 text-center bg-blue-50 group-hover:bg-blue-100 transition">
            <div v-if="selectedFiles.length === 0" class="text-blue-600">
              <span class="text-2xl block">+</span>
              <span class="text-xs font-medium">点击选择文件</span>
            </div>
            <div v-else class="text-blue-700 text-sm font-medium">
              已选 {{ selectedFiles.length }} 个文件
            </div>
          </div>
        </div>
        <button
            @click="uploadAndBuild"
            :disabled="isLoading || selectedFiles.length === 0"
            class="mt-3 w-full bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium py-2 rounded shadow disabled:bg-gray-300 disabled:cursor-not-allowed transition"
        >
          {{ isLoading ? '入库中...' : '立即入库' }}
        </button>
        <p v-if="buildMsg" class="mt-2 text-xs text-center truncate" :class="buildType === 'error' ? 'text-red-500' : 'text-green-600'">
          {{ buildMsg }}
        </p>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-4 bg-gray-50">
        <div class="flex justify-between items-center mb-3">
          <h3 class="text-xs font-semibold text-gray-500 uppercase tracking-wider">已入库文档 ({{ docList.length }})</h3>
        ...
        <ul v-if="docList.length > 0" class="space-y-2">
          <li v-for="file in docList" :key="file" class="group flex items-center justify-between bg-white p-2.5 rounded border border-gray-200 hover:border-blue-300 hover:shadow-sm transition-all">
            <div class="flex items-center overflow-hidden mr-2">
              <span class="text-gray-400 mr-2 text-xs flex-shrink-0">📄</span>
              <span class="text-sm text-gray-700 truncate" :title="file">{{ file }}</span>
            </div>
            <button
                @click="deleteDocument(file)"
                class="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-600 hover:bg-red-50 p-1.5 rounded transition-all duration-200 flex-shrink-0"
                title="删除文档"
            >
        ...
        <div v-else class="text-center text-gray-400 text-sm mt-10 italic">
          暂无文档，请上传
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 flex flex-col bg-white relative">
      ...
      <div class="h-16 border-b border-gray-100 flex items-center justify-between px-8 shadow-sm z-10 bg-white">
        <div class="flex items-center">
          <h1 class="text-xl font-bold text-gray-800 flex items-center">
            <span class="mr-2">🤖</span> 智能助手
          </h1>
          <span v-if="sessionId" class="ml-3 text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full border border-green-200">
            会话中 ({{ Math.floor(messages.length / 2) }} 轮)
          </span>
        </div>
        ...
        <div class="flex space-x-2">
          ...
          <button @click="clearContext" :disabled="!sessionId" class="text-xs text-red-500 hover:text-red-700 border border-red-100 px-3 py-1.5 rounded hover:bg-red-50 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center">
            <span class="mr-1">🗑️</span> 清空历史
          </button>
        </div>
      </div>
      ...
      <div ref="chatContainer" class="flex-1 overflow-y-auto p-8 space-y-6 bg-gray-50/50 scroll-smooth">
        ...
        <div v-if="messages.length === 0" class="h-full flex flex-col items-center justify-center text-gray-400 opacity-80">
          ...
          <p class="text-sm mt-2 mb-6">支持文本、图片提问，基于知识库回答</p>
          ...
          <div class="flex flex-wrap justify-center gap-2 max-w-2xl">
            <button
                ...
                :key="idx"
                @click="sendMessage(q)"
                class="px-4 py-2 bg-white border border-blue-200 text-blue-600 text-xs rounded-full hover:bg-blue-50 hover:border-blue-400 hover:shadow-md transition flex items-center"
            >
              <span class="mr-1">⚡</span> {{ q }}
            </button>
          </div>
        </div>
        ...
        <div v-for="(msg, index) in messages" :key="index" class="flex w-full" :class="msg.role === 'user' ? 'justify-end' : 'justify-start'">
          ...
          <div v-if="msg.role !== 'user'" class="flex max-w-[85%] items-start gap-3">
            ...
            <div class="bg-white text-gray-800 p-4 rounded-2xl rounded-tl-none shadow-sm border border-gray-100 text-sm leading-relaxed flex flex-col gap-2">
              ...
              <div v-if="msg.sources && msg.sources.length > 0" class="mt-2 pt-2 border-t border-gray-100">
                <p class="text-xs font-semibold text-gray-500 mb-1 flex items-center">
                  <span class="mr-1">📚</span> 参考来源:
                </p>
                <ul class="space-y-1">
                  <li v-for="(src, idx) in msg.sources" :key="idx" class="text-xs bg-gray-50 p-2 rounded border border-gray-200 hover:bg-blue-50 transition cursor-default">
                    <div class="flex justify-between items-center">
                      <span class="font-medium text-blue-700 truncate max-w-[200px]" :title="src.source">{{ src.source }}</span>
                      <span class="text-gray-400 ml-2 whitespace-nowrap">相似度: {{ src.score }}</span>
                    </div>
        ...
        <!-- 加载中状态 -->
        <div v-if="isChatLoading" class="flex justify-start w-full">
        ...
        <div class="max-w-4xl mx-auto relative">
          ...
          <div v-if="imagePreviewUrl" class="absolute bottom-20 left-0 bg-white p-3 rounded-xl shadow-lg border border-gray-200 flex items-center gap-3 animate-fade-in-up z-20">
            ...
            <div class="flex flex-col">
              ...
              <span class="text-[10px] text-gray-500">发送后将自动清除</span>
            </div>
          </div>
          ...
          <div class="flex items-end gap-2">
            ...
            <div class="relative pb-2">
              ...
              <button
                  ...
                  class="h-12 w-12 flex items-center justify-center rounded-full text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition"
                  title="上传图片"
            ...
            <div class="flex-1 relative">
              ...
              type="text"
              placeholder="请输入问题，或上传图片提问..."
              class="w-full border border-gray-300 rounded-full pl-6 pr-14 py-4 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm text-sm transition"
              :disabled="isChatLoading"
              />
              <button
                  @click="sendMessage()"
              :disabled="isChatLoading || (!userInput.trim() && !selectedImage)"
              class="absolute right-2 top-2 bottom-2 bg-blue-600 hover:bg-blue-700 text-white rounded-full px-6 font-medium disabled:bg-gray-300 disabled:cursor-not-allowed transition transform active:scale-95"
              >
              发送
              </button>
            </div>
          ...
          <div class="text-center mt-3">
            <p class="text-xs text-gray-400">内容由 AI 生成，图片识别依赖视觉模型，请仔细甄别</p>
          </div>
        </div>
      </div>
    </div>
...
const sendMessage = async (customQuestion = null, event = null) => {
  // 如果 customQuestion 是事件对象（说明是回车触发的且没传参），则取 userInput
  let q = "";
  if (typeof customQuestion === 'string') {
    ...
    // 其他情况（如点击发送按钮，customQuestion 为 null）
    q = userInput.value.trim();
  }
  ...
  clearImage(); // 立即清除 UI 状态

  isChatLoading.value = true;

  try {
    let payload = {
      question: q || "请分析这张图片", // 确保 question 永远是字符串
    ...
    // 【修复点 3】优化错误信息显示，避免显示 [object Object]
    let errorMsg = '服务不可用';
    if (e.response) {
      // 422 或其他 HTTP 错误
      errorMsg = e.response.data.detail || JSON.stringify(e.response.data);
    } else if (e.message) {
      errorMsg = e.message;
    }
    addMessage('assistant', `❌ 错误：${errorMsg}`);
    console.error(e); // 在控制台打印详细错误以便调试
  } finally {
...
const clearContext = async () => {
  ...
  try {
    ...
    messages.value = [];
    sessionId.value = null; // 重置 ID，下次发送会创建新会话
    showToast("对话历史已清空");
  } catch (e) {
